...
export default function BillingPage() {
  ...
  const fetchUserAndTransactions = async () => {
    try {
      const [resUser, resTx, resConfig] = await Promise.all([
        fetch("/api/auth/me"),
    ...
    if (typeof window !== "undefined") {
      ...
      if (sessionId && params.get("success") === "true") {
        fetch("/api/billing/verify-session", {
          ...
          .then((res) => res.json())
          .then((data) => {
            if (data.verified) fetchUserAndTransactions();
          })
          .catch(() => {});
      }
    }
  ...
  const handleExecuteCheckout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchUserAndTransactions();
      setTimeout(() => setSelectedPlanForUpgrade(null), 2500);
    } catch {
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500 text-sm">Loading billing details...</div>;
  }
    ...
    <div className="space-y-8 max-w-5xl pb-12">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-slate-400 mt-1">
          Manage your current plan, payment history, and account limits.
        </p>
      </div>

      {subscriptionNotice && userPlan === "INACTIVE" && (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-400 font-medium pt-1">
              Registered email: <span className="text-slate-200 font-mono">{userEmail}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      {userPlan !== "LTD" && (
        <div className="space-y-4">
          <h3 className="text-lg font-extrabold text-white flex items-center gap-2">
            ...
            {userPlan === "PRO" ? "Upgrade Options" : "Choose Your Plan & Pay"}
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            ...
            {userPlan !== "PRO" && (
              <div className="p-6 rounded-3xl bg-[#0b101d]/90 border border-slate-800 flex flex-col justify-between space-y-6 hover:border-slate-700 transition-colors">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex items-baseline gap-1">
                      <span className="text-4xl font-extrabold text-white">${planPrices.PRO}</span>
                ...
                >
                  Subscribe ${planPrices.PRO}/Month
                </button>
              </div>
            )}
      ...
      {userPlan !== "LTD" && (
        <div className="p-6 rounded-3xl bg-[#0b101d]/90 border border-slate-800 space-y-4">
          ...
          <p className="text-xs text-slate-400">
            Have you purchased an offline Lifetime deal or received a gift code from the admin? Enter it below.
          </p>
          ...
          {redeemStatus && (
            <div className={`p-3.5 rounded-xl text-xs flex items-center gap-2 border ${
              ...
              {redeemStatus.type === "success" ? <CheckCircle2 className="w-4 h-4 shrink-0 text-emerald-400" /> : <AlertCircle className="w-4 h-4 shrink-0 text-rose-400" />}
          ...
          <form onSubmit={handleRedeemCode} className="flex flex-col sm:flex-row gap-3 max-w-xl">
            <input
              ...
              placeholder="e.g. BZ-LTD-PRO-2026"
      ...
      {selectedPlanForUpgrade && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0c1220] border border-white/10 rounded-3xl max-w-md w-full p-6 sm:p-8 space-y-6 shadow-2xl relative">
            <button onClick={() => setSelectedPlanForUpgrade(null)} className="absolute top-5 right-5 p-2 rounded-xl text-slate-400 hover:text-white hover:bg-white/5 transition-colors"><X className="w-5 h-5" /></button>
            <div className="space-y-1">
              <span className="text-xs font-mono font-bold uppercase text-indigo-400">Secure Checkout</span>
              <h3 className="text-xl font-extrabold text-white">Upgrade to {selectedPlanForUpgrade === "LTD" ? "Lifetime Founder Pass" : "Pro Monthly"}</h3>
              <p className="text-xs text-slate-400">Total due: <span className="text-white font-bold text-sm">${planPrices[selectedPlanForUpgrade]} USD</span></p>
            </div>
            {checkoutError && (
              <div className="p-3.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-300 text-xs flex items-center gap-2">
                <AlertCircle className="w-4 h-4 shrink-0" /><span>{checkoutError}</span>
              </div>
            )}
            {checkoutSuccess && (
              <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-xs flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 shrink-0" /><span>{checkoutSuccess}</span>
              </div>
            )}
            ...
            <form onSubmit={handleExecuteCheckout} className="space-y-4">
              {checkoutMethod === "CARD" ? (
                <>
                  <div><label className="block text-xs font-semibold text-slate-300 mb-1">Cardholder</label><input type="text" required value={cardHolder} onChange={(e) => setCardHolder(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-xs text-white focus:outline-none focus:border-indigo-500" /></div>
                  <div><label className="block text-xs font-semibold text-slate-300 mb-1">Card Number</label><div className="relative"><CreditCard className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" /><input type="text" required value={cardNumber} onChange={(e) => setCardNumber(e.target.value)} placeholder="4242 •••• •••• 4242" className="w-full pl-10 pr-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-xs text-white font-mono focus:outline-none focus:border-indigo-500" /></div></div>
